...
def input_until_correct_download_range(default_prompt: str, error_prompt: str) -> str:
    """
    입력값이 공백 또는 "숫자" 또는 "숫자-숫자" 만 입력할때까지 반복하여 입력받고, 입력받은 값을 리턴
    ...
    while (True):
        if (dialog == "") or (dialog.isdigit() or (dialog.find('-') != -1 and dialog.split('-')[0].isdigit() and dialog.split('-')[1].isdigit())):
            break
        else:
